...
function AcademicaPage({ tenant }) {
  ...
  return (
    <div className="min-h-screen bg-[#eef6f1] text-slate-950" style={{ backgroundImage: `url('${tenant.assetPath}/background.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-lg">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              <img src={`${tenant.assetPath}/logo.png`} alt="Logo" className="h-14 w-14 rounded-3xl border border-slate-200 bg-white/80" />
              <div>
                <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Academia</p>
                <p className="text-2xl font-semibold text-slate-950">{tenant.companyName}</p>
              </div>
            </div>
            <nav className="flex flex-wrap gap-3 text-sm uppercase tracking-[0.25em] text-slate-600">
              {tenant.menu.map((item) => (
                <a key={item.href} href={item.href} className="rounded-full border border-slate-200 bg-slate-50 px-4 py-3 transition hover:border-slate-300 hover:bg-slate-100">
                  {item.label}
                </a>
              ))}
            </nav>
      ...
      <main className="relative mx-auto px-6 pb-24 sm:px-10 lg:max-w-7xl">
        <section className="relative overflow-hidden rounded-[1.8rem] border border-slate-200 bg-white/90 shadow-xl">
          ...
          <div className="relative grid gap-10 py-20 px-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:px-12">
            <div className="space-y-6">
              ...
              <p className="max-w-xl text-lg leading-8 text-slate-700">{tenant.heroText}</p>
              <p className={`max-w-xl text-lg leading-8 ${accent.highlight}`}>{tenant.heroHighlight}</p>
              <div className="flex flex-wrap gap-4">
                {tenant.buttons.map((button) => (
                  <a key={button.href} href={button.href} className={`inline-flex items-center justify-center rounded-2xl px-6 py-3 text-sm font-semibold shadow-lg transition ${button.primary ? accent.button : `border border-emerald-700 bg-slate-50 text-slate-900 ${accent.secondary}`}`}>
                    {button.label}
                  </a>
                ))}
              </div>
            </div>
            <div className="grid gap-6 rounded-[1.75rem] border border-slate-200 bg-slate-50/95 p-6 shadow-lg backdrop-blur-sm">
        ...
        <section className="mt-16 grid gap-6 md:grid-cols-3">
          {tenant.cards.map((item) => (
            <article key={item.title} className="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-8 shadow-lg transition hover:-translate-y-1 hover:shadow-2xl">
              ...
              <p className="mt-4 text-slate-700">Material didáctico con entrega rápida y diseño ajustado al ciclo escolar.</p>
            </article>
          ))}
        </section>
        ...
        <section id="servicios" className="mt-20 space-y-8">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-lg leading-8 text-slate-700">Creamos cuadernillos, guías y carpetas con producción express y atención al detalle académico.</p>
          </div>

          <div className="grid gap-6 lg:grid-cols-3">
            {tenant.services.map((service) => (
              <div key={service.title} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-7 shadow-lg transition hover:-translate-y-1 hover:shadow-2xl">
        ...
        <section className="mt-16 rounded-[2rem] border border-emerald-400/40 bg-emerald-50/90 p-8 shadow-lg">
          <div className="grid gap-8 lg:grid-cols-[1.3fr_1fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold text-slate-950">Encuéntranos en el barrio educativo</h2>
